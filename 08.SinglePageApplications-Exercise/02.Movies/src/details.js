...
async function getMovie(id) {
    ...
    // add one more request if there is a user
...
function createDetails(movie, likes, hasLiked) {
    ...
    if (userData != null) {
        if (userData.id == movie._ownerId) {
            controls.appendChild(e('a', { className: 'btn btn-danger', href: "#" , onClick: () => showDelete(movie._id) },'Delete'));
            controls.appendChild(e('a', { className: 'btn btn-warning', href: "#",  onClick: () => showEdit(movie._id, movie.title, movie.description, movie.img) }, 'Edit'));
        } else {
